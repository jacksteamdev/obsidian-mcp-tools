...
- **`plugin.loadData()` / `plugin.saveData()` are NOT atomic** — the default Obsidian persistence primitives are two independent async calls, so any feature that does `load → modify → save` in response to concurrent events MUST serialize the critical section with a mutex. The command-permissions feature learned this the hard way in Fase 2 of issue #29: the naive Fase 1 handler dropped 32 of 35 audit entries under a 35-parallel-curl smoke test because all the readers read the same "before" state and the last writer clobbered the rest. Fix: `features/command-permissions/services/settingsLock.ts` exposes a minimal async mutex with a 35-way regression test; the handler wraps the fast-path load/save in phase A (lock), the modal wait in phase AB (no lock), and the final persist in phase B (lock). **Rule**: if you add a new feature that writes to plugin settings in response to an HTTP route or an event listener, use a mutex — do not assume the events are naturally serialized.
...
- **Counts as of 2026-04-11 fork state**: `mcp-server` 93 pass / 8 files; `obsidian-plugin` 126 pass / 9 files. Total **219 pass**.
